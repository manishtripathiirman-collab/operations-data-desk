...
# 2. LOAD DATA DIRECTLY FROM EXCEL
@st.cache_data
def load_data():
...
with tab2:
    ...
    if not s_rent.empty:
        ...
        for y in yrs: 
            s_rent[f"{y}_PSF"] = s_rent[y] / s_rent["Est_SqFt"]
...
with tab3:
    ...
    if b_yr == c_yr: 
        st.warning("Please choose two different fiscal periods.")
    else:
        ...
        if not p_seasoned.empty:
            ...
            c_pv["B_Rev"] = c_pv[(b_yr, "Rev")] / c_pv["SqFt"]
            c_pv["B_Rent"] = c_pv[(b_yr, "Rent")] / c_pv["SqFt"]
            c_pv["C_Rev"] = c_pv[(c_yr, "Rev")] / c_pv["SqFt"]
            c_pv["C_Rent"] = c_pv[(c_yr, "Rent")] / c_pv["SqFt"]
